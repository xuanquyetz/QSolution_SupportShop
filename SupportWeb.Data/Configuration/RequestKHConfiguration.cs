...
namespace SupportWeb.Data.Configuration
{
    public class RequestKHConfiguration : IEntityTypeConfiguration<RequestKH>
    {
        public void Configure(EntityTypeBuilder<RequestKH> builder)
        {
            ...
            builder.Property(q => q.FormThucHien).HasMaxLength(200);
            builder.Property(q => q.NguoiYeuCau).HasMaxLength(100);
            builder.Property(q => q.GhiChu).HasMaxLength(500);
            builder.HasOne(q => q.KhachHang).WithMany(x => x.RequestKH).HasForeignKey(x => x.KhachHangMa);
            builder.HasOne(q => q.NhanSu).WithMany(x => x.RequestKH).HasForeignKey(x => x.KyThuatMa);
            builder.HasOne(q => q.NhanSu).WithMany(x => x.RequestKH).HasForeignKey(x => x.CodeMa);
        }
    }
}
